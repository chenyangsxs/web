...
CyWeb::CyWeb() {
    ...
    strcat(m_root, root);
    m_Sql.m_user = "root";
    m_Sql.m_passWord = "777541";
    m_Sql.m_databaseName = "WebDB";
    m_Sql.m_sql_num = 5;
...
void CyWeb::PrepareSql(){
    //用RAII方式封装mysql资源 这里没有采用这种方式，而是手动的方式
    Sqlconnect* m_sqlconnect = Sqlconnect::GetInstance();
    m_sqlconnect->init("localhost", m_Sql.m_user, m_Sql.m_passWord, m_Sql.m_databaseName, 3306, m_Sql.m_sql_num);
    MYSQL *mysql = m_sqlconnect->GetOneConnect();
    ...
    m_sqlconnect->ReleaseConnection(mysql);
